...
class m_nilai_smart extends Model
{
    ...
    public function editData($m_alternatif_id, $m_kriteria_id, $nilai_awal)
    {
        DB::table('m_nilai_smarts')->where('m_alternatif_id', $m_alternatif_id)->where('m_kriteria_id', $m_kriteria_id)->update(['nilai_awal' => $nilai_awal]);
    }
    ...
    {
        return m_nilai_smart::with('Alternatif', 'Kriteria')->rightjoin('m_alternatifs', 'm_alternatifs.id', '=','m_nilai_smarts.m_alternatif_id')->where('m_alternatifs.user_id', Auth::user()->id)->where('nilai_awal', null)->get();
    }
}
